...
top_day_revenue = find_top_entry(daily_summary, "revenue")
top_day_units = find_top_entry(daily_summary, "units_sold")

print_day_table(daily_summary)
print()
print(f"Total Revenue:          ${total_revenue:,.2f}")
print(f"Busiest day by revenue: {top_day_revenue['date']} (${top_day_revenue['revenue']:,.2f})")
print(f"Busiest day by units:   {top_day_units['date']} ({top_day_units['units_sold']} units)")
